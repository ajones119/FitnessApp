...
const ExerciseRow = ({exercise, doubleUnilateral = false, disabled = false}: ExerciseRowProps) => {
    ...
    return <motion.div
        ...
        exit={{ opacity: 0, x: 80 }}
        >
        <Checkbox disabled={disabled} checked={exercise.completed} onCheckedChange={() => mutateExercise([{...exercise, completed: !exercise?.completed}])} className="pt-4" />
        {
            exerciseTemplate.unilateral && !doubleUnilateral && <div>
                <p className="text-xs pb-1 text-accent">(L)Reps</p>
                <Input disabled={disabled} value={editRepsLeft} onChange={(e) => setEditRepsLeft(e.target.value)} onBlur={(e) => mutateExercise([{...exercise, leftReps: parseInt(e.target.value)}])} />
            </div>
        }
        {
            exerciseTemplate.unilateral && !doubleUnilateral && <div>
                <p className="text-xs pb-1 text-accent">(L)Weight</p>
                <Input disabled={disabled} value={editWeightLeft} onChange={(e) => setEditWeightLeft(e.target.value)} onBlur={(e) => mutateExercise([{...exercise, leftWeight: parseInt(e.target.value)}])} />
            </div>
        }
        <div>
            <p className="text-xs pb-1 text-accent">Reps</p>
            <Input disabled={disabled} value={editReps} onChange={(e) => setEditReps(e.target.value)} onBlur={(e) => {
        ...
        <div>
            ...
            <Input disabled={disabled} value={editWeight} onChange={(e) => setEditWeight(e.target.value)} onBlur={(e) => {
